...
import { KeyAccessFormData } from "./KeyAccessRequisitionForm";
...
export default function KeyConfirmationModal({
  ...
  const dateDetails: { label: string; value: string }[] = [
    { label: "Issuance Date", value: formData.issuanceDate },
  ];
    ...
    <div className="mx-auto max-w-3xl rounded-3xl border border-gray-100 bg-white/65 p-10 shadow-[0_24px_48px_rgba(160,60,60,0.10)] backdrop-blur-2xl">
      ...
      <div className="mb-6">
        ...
        <p className="mt-1 text-[13px] text-[#7c5a5a]">
          Please review your Key & Access requisition before submitting.
        </p>
      </div>
            ...
            {dateDetails.map(({ label, value }) => (
              <div key={label} className="flex justify-between text-[13px]">
                ...
                <span className="font-medium text-[#1e1b1b]">
                  {value ? dateFormatter(value) : "Not Selected"}
                </span>
              </div>
            ))}
          </div>
